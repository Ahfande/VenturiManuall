...
// Button L1P
// OFF
const BtnL1POFF = document.getElementById("btnL1POFF").addEventListener('click', () =>{
    database.ref("Manual/L1P/")
    .set({ status: "OFF" })
    .then(() => {
    console.log("L1P OFF");
...
const BtnL1PON = document.getElementById("btnL1PON").addEventListener('click', () =>{
    database.ref("Manual/L1P/")
    .set({ status: "ON" })
    .then(() => {
    console.log("L1P ON");
...
window.onload = (() => {
    database.ref("Manual/L1P/").get().then((snapshot) => {
